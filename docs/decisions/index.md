...
| ADR-0008 | Accepted | Use SVG as the initial presentation asset format for the process/PFD layer: presentation-only symbol assets (never semantic domain objects) with a uniform `viewBox="0 0 100 100"`; embedded machine-readable generic ordered `anchor-in-N` / `anchor-out-N` slots distinct from semantic `ProcessPort` ids; input/output roles derived from `ProcessStream` incidence; DeepPlant-original provenance (AGPL-3.0-only, `reference` / `candidate-alignment` standards state); engineering-diagram visual language; renderer, layout, routing, labels, rotation, and custom-symbol machinery deferred. |
